<?php

return [
    'top' => [
        'big_title' => 'Онлайн школа программирования',
        'second_title' => 'Индивидуальные компьютерные курсы для детей и взрослых онлайн от 8 до 35 лет',
        'try_button' => 'Попробуйте бесплатно'
    ],
    'about' => [
        'small_title' => 'О нас',
        'big_title' => 'Первый выбор для онлайн-образования в любом месте',
        'description' => 'Мы молодая команда энтузиастов преподавателей, которая объединилась для того, чтобы дать лучшее образование в сфере IT для вас или для вашего ребенка',
        'available_subjects' => 'доступных тем',
        'online_courses' => 'онлайн курсов',
        'skilled_instructors' => 'опытных преподавателей',
        'happy_students' => 'счастливых студентов',
    ],
    'advantages' => [
        'small_title' => 'Почему именно мы?',
        'title' => 'Почему вам стоит начать учиться у нас','description' => 'Aliquyam accusam clita nonumy ipsum sit sea clita ipsum clita, ipsum dolores amet voluptua duo dolores et sit ipsum rebum, sadipscing et erat eirmod diam kasd labore clita est. Diam sanctus gubergren sit rebum clita amet.',
        'block1_title' => 'Опытные преподаватели',
        'block1_description' => 'Наши преподаватели - специалисты с В нашей школе работают преподаватели с большим педагогическим опытом и.',
        'block2_title' => 'Сертификат',
        'block2_description' => 'После успешного окончания курса обучения, вы получаете официальный сертификат нашей школы.',
        'block3_title' => 'Онлайн обучение',
        'block3_description' => 'Labore rebum duo est Sit dolore eos sit tempor eos stet, vero vero clita magna kasd no nonumy et eos dolor magna ipsum.',
    ],
    'courses' => [
        'small_title' => 'Our Courses',
        'big_title' => 'Checkout New Releases Of Our Courses',
    ],
    'course_carousel' => [
        'details' => 'Подробнее'
    ],
    'signup_form' => [
        'title' => 'Запишитесь на пробный урок',
        'name' => 'Ваше имя',
        'phone' => 'Ваш телефон',
        'selector' => 'Виберите курс',
        'button' => 'Записаться сейчас',
    ],
    'send_message_form' => [
        'small_title' => 'Нужна помощь?',
        'big_title' => 'Отправьте нам сообщение',
        'name' => 'Ваше имя',
        'phone' => 'Ваш телефон',
        'message' => 'Виберите курс',
        'button' => 'Отправить',
        'error' => 'Ошибка',
        'location_title' => 'Где мы находимся',
        'phone_title' => 'Позвоните нам',
        'email_title' => 'Email',
    ],
    'successfull_sending' => 'Мы свяжемся с вами в ближайшее время',
];
